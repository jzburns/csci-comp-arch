...
After the loop, a branch-with-link instruction stores the return address in `lr`. The next instruction attempts to push `lr` onto the stack using `str lr, [sp, #-4]!`. Since the stack pointer is never initialized, it starts at address `0x00000000`. This causes an invalid memory access at `0xFFFFFFFC`, leading to a program crash in CPulator. https://youtube.com/UCD8Xa-wnhXADTy9pxXuykwQ/video/upload?filter=%5B%5D&sort=%7B%25columnType%22%3A%22date%22%2C%-%22%3A%22DESCENDING%22%7D
