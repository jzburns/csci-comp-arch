...
\vspace{0.3cm}
{\color{lightblue}\normalsize Abstraction Layers, ISA vs.\ Microarchitecture, Stored-Program Concept, and Memory Models in AArch64\par}
\vspace{0.6cm}
{\color{softgray}\small Computer Architecture \& Organization \quad\textbar\quad Topic 1\par}
\end{minipage}
\end{frame}
...
\begin{frame}{Topic 1 Technical Roadmap}
    ...
    \vspace{0.4em}
    \begin{itemize}
        \item \key{Sub-topic 1}: The Full Stack of Abstraction Layers
        \item \key{Sub-topic 2}: Computer Architecture versus Computer Organization
        \item \key{Sub-topic 3}: Instruction Set Architecture (ISA) versus Microarchitecture
        \item \key{Sub-topic 4}: The Stored-Program Concept \& Execution Cycle
        \item \key{Sub-topic 5}: Memory Models: von Neumann, Harvard, and Modified Harvard
        \item \key{Sub-topic 6}: Grand Synthesis \& Interactive Self-Test Review
    \end{itemize}
\end{frame}
...
\begin{frame}{Real-World Continuity: The ARM64 Ecosystem}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            \begin{block}{Organizational Evolution}
                \begin{itemize}
                    \item Early Cores: In-order or narrower out-of-order execution pipelines.
                    \item Modern Cores: Wide 8--9 issue out-of-order engines, deep multi-level caches, advanced matrix coprocessors.
                \end{itemize}
            \end{block}
        \end{column}
    \end{columns}
...
\begin{frame}{Visualizing the Microarchitectural Datapath}
    \begin{center}
        \begin{tikzpicture}[scale=0.9, every node/.style={font=\footnotesize}]
            % Register File (Expanded Size)
            \draw[thick, fill=navy, text=white] (0,-0.2) rectangle (3.5,3.6);
            \node[text=white, font=\small\bfseries, align=center] at (1.75,1.7) {64-Bit Register\\File};
            ...
            % ALU
            \draw[thick, fill=deepblue, text=white] (6.0,0.8) -- (8.2,1.8) -- (8.2,2.8) -- (6.0,3.8) -- (6.0,2.6) -- (6.5,2.3) -- (6.0,2.0) -- cycle;
            \node[text=white, font=\small\bfseries] at (7.1,2.3) {64-Bit ALU};
            
            % Routing Lines
            \draw[thick, -Latex, draw=navy] (3.5,3.1) -- node[above]{\code{rd1 (x2)}} (6.0,3.3);
            \draw[thick, -Latex, draw=navy] (3.5,2.3) -- node[below]{\code{rd2 (x3)}} (6.0,1.3);
            \draw[thick, -Latex, draw=navy] (8.2,2.3) -- (9.2,2.3) -- (9.2,-0.6) -- (1.75,-0.6) -- node[left]{\code{alu\_result}} (1.75,-0.2);
            
            % Control signals
            \node[draw=gold, fill=lightblue, rounded corners=2pt] at (7.1,4.3) {\code{alu\_control = add}};
            \draw[dashed, -Latex, draw=gold!70!black] (7.1,4.0) -- (7.1,3.1);
        \end{tikzpicture}
    \end{center}
    \begin{itemize}
