...
% --- Slide 36: Wrap-Up & Transition to Topic 2 ---
\begin{frame}{Wrap-Up and Next Steps}
    \begin{block}{Transition to Topic 2}
        Building upon the foundational abstraction layers, system organization, and memory models, the next module examines:
        \begin{itemize}
            \item \key{RISC Design Principles}: Exploring the philosophy of Reduced Instruction Set Computers, simpler control circuitry, and high energy efficiency.
            \item \key{The Load/Store Paradigm}: Examining how RISC architectures strictly isolate memory access from register-based computation.
            \item \key{CISC versus RISC Trade-Offs}: Contrasting variable-length instructions and direct memory arithmetic with fixed 32-bit execution models.
            \item \key{AArch64 Architectural Overview}: Analyzing the modern 64-bit ARM instruction encoding model and dual register views (\code{W} vs. \code{X}).
            \item \key{Programmer-Visible State}: Surveying general-purpose registers (\code{x0}--\code{x30}), stack pointers, link registers, and \code{PSTATE} condition flags.
        \end{itemize}
    \end{block}
\end{frame}
