...
For a fixed workload, performance is a completion rate: how many copies of that workload finish per second.
\[
\text{Performance}=\frac{1\ \text{completed task}}{\text{Execution Time}}
\quad [\text{tasks/s}]
\]
For example, if one program run takes $1$ second, performance is $1$ completed run per second.
\end{block}
\vspace{0.05cm}
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
...
\item Example: completed tasks per second.
\end{itemize}
\end{block}
\end{columns}
\end{frame}
...
CPI is the average number of clock cycles used per executed instruction for a particular workload.
\[
...
\vspace{0.15cm}
\begin{itemize}
\item A lower CPI usually means fewer cycles are needed per instruction.
\item CPI is an average: stalls, cache misses, and branch penalties can increase it.
\item CPI is not fixed by the ISA alone; it depends on the processor and workload.
\end{itemize}
\end{frame}
