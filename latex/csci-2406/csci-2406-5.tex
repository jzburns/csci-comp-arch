...
\begin{frame}{Shift Amount Constraints and Boundaries}
    Shift operations in AArch64 adhere to strict architectural limits:
    \vspace{0.3em}
    \begin{itemize}
        \item For 32-bit (\code{W}) register operations, shift amounts must range strictly from $0$ to $31$.
        \item For 64-bit (\code{X}) register operations, shift amounts must range strictly from $0$ to $63$.
        \item \key{Architectural Violation}: Shifting by an amount equal to or exceeding the register width ($32$ for W, $64$ for X) is illegal and triggers assembler errors or undefined behavior.
    \end{itemize}
    \vspace{0.2em}
...
\begin{frame}{Logical Immediates and Bitmask Encoding}
    ...
    \vspace{0.2em}
    \begin{itemize}
        \item Logical immediates are not arbitrary numbers; they are encoded as a replicated bit pattern across element sizes of $2, 4, 8, 16, 32,$ or $64$ bits.
        \item \key{Assembler Restriction}: If a constant cannot be expressed as a repeating bitmask pattern, the assembler rejects it with an error (e.g., ``immediate cannot be encoded'').
        \item Enables common bitmasks and flag selectors to be embedded directly within single-cycle logical instruction words.
    \end{itemize}
\end{frame}
...
\begin{frame}{Topic 5 Comprehensive Summary}
    \begin{itemize}
        ...
        \item \key{Shift Operations}: Support scaling via \code{lsl}, \code{lsr}, \code{asr}, and \code{ror}, with strict width boundaries and hardware barrel-shifter integration.
        \item \key{Constant Generation}: Managed through 12-bit arithmetic immediates, bitmask encoding rules, and wide moves (\code{movz}/\code{movk}).
    \end{itemize}
\end{frame}
...
\begin{frame}{Self-Test: Questions 3 \& 4}
    \begin{block}{Question 3: Shift Limits and the Barrel Shifter}
        What are the architectural shift limits for 32-bit and 64-bit registers in AArch64, and what is the primary advantage of the built-in barrel shifter?
    \end{block}
    \vspace{0.4em}
    \begin{block}{Question 4: Constant Loading and Bitmasks}
        Why might the assembler reject a logical immediate constant with an error, and how do compilers load wide 64-bit constants?
    \end{block}
\end{frame}
...
\begin{frame}{Self-Test: Solutions 3 \& 4}
    \begin{block}{Solution 3}
        Shift amounts must range from $0$ to $31$ for W registers and $0$ to $63$ for X registers; shifting by or above the register width is illegal. The barrel shifter allows a source operand to be shifted as part of a datapath instruction in a single cycle.
    \end{block}
    \vspace{0.3em}
    \begin{block}{Solution 4}
        The assembler rejects constants that cannot be expressed as repeating bitmask patterns. Compilers construct wide 64-bit constants using wide move sequences like \code{movz} followed by one or more \code{movk} instructions.
    \end{block}
\end{frame}
...
\begin{frame}{Wrap-Up and Next Steps}
    \begin{block}{Transition to Topic 6}
        Building upon arithmetic and logical instruction execution, the next module examines:
        \begin{itemize}
            \item \key{Data Transfer and Addressing}: Advanced memory management and data movement mechanics.
            \item \key{Load and Store Instructions}: Transferring data efficiently between memory and the register file.
            \item \key{Addressing Modes}: Exploring base-register, offset, pre-index, and post-index pointer calculations.
            \item \key{Memory Access and Alignment}: Handling byte/word sizing and natural alignment requirements.
            \item \key{Arrays and Pointers}: Managing contiguous data structures and pointer arithmetic in AArch64.
        \end{itemize}
    \end{block}
\end{frame}
