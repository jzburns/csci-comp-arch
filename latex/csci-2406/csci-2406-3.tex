...
\vspace{0.6cm}
{\color{softgray}\small Computer Architecture \& Organization \quad\textbar\quad Topic 3\par}
\end{minipage}
\end{frame}

% --- Slide 2: Topic Roadmap ---
\begin{frame}{Topic 3 Technical Roadmap}
    \begin{block}{Topic Objective}
        Examine how machine-level information is encoded, interpreted, and organized across fixed-width registers and byte-addressed memory structures.
    \end{block}
    \vspace{0.4em}
    \begin{itemize}
...
% --- Slide 14: Unsigned Arithmetic Wraparound ---
\begin{frame}{Unsigned Arithmetic and Wraparound}
    Unsigned operations are evaluated modulo $2^n$ for an $n$-bit container:
    \vspace{0.2em}
    \begin{itemize}
        \item \key{Overflow Behavior}: If an arithmetic operation exceeds the maximum representable value ($2^n - 1$), high-order carry bits are discarded.
        \item \key{Wraparound}: The stored result wraps around circularly within the fixed container width.
        \item Processors record unsigned carry-out conditions in status flags.
    \end{itemize}
    \vspace{0.3em}
    \begin{block}{Classic Wraparound Example}
        In 8-bit unsigned arithmetic:
        $$255 + 1 = 1111\,1111_2 + 1 = 0000\,0000_2 = 0$$
    \end{block}
\end{frame}
...
\begin{frame}{Two's Complement in Action: $-9 + 6$}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.9, every node/.style={font=\small}]
            % Input boxes
            \node[draw=navy, fill=lightblue, text=darkgray, rounded corners=2pt, text width=4.0cm, align=center] at (0,2.2) {
                \textbf{$-9$ (5-bit Two's Complement)}\\
                \code{10111}
            };
            \node[draw=navy, fill=lightblue, text=darkgray, rounded corners=2pt, text width=4.0cm, align=center] at (6.0,2.2) {
                \textbf{$+6$ (5-bit Positive)}\\
                \code{00110}
            };
            
            % Adder box
            \node[draw=navy, fill=navy, text=white, rounded corners=3pt, text width=5.0cm, align=center, minimum height=0.8cm] at (3.0,0.8) {
                \textbf{Simple Hardware Adder}
            };
            
            % Output box
            \node[draw=gold, fill=gold!20, text=navy, rounded corners=2pt, text width=4.0cm, align=center] at (3.0,-0.6) {
                \textbf{Result: $-3$}\\
                \code{11101}
            };
            
            % Arrows
            \draw[thick, -Latex, draw=navy] (0,1.6) -- (2.0,1.2);
            \draw[thick, -Latex, draw=navy] (6.0,1.6) -- (4.0,1.2);
            \draw[thick, -Latex, draw=navy] (3.0,0.4) -- (3.0,0.0);
        \end{tikzpicture}
    \end{center}
    \vspace{-0.1em}
    \begin{itemize}
...
\begin{frame}{Topic 3 Comprehensive Summary}
    \begin{itemize}
        ...
        \item \key{Unsigned Integers}: Represent non-negative values with strict wraparound modulo $2^n$.
...
\begin{frame}{Self-Test: Questions 1 \& 2}
    ...
    \end{block}
    \vspace{0.4em}
    \begin{block}{Question 2: Unsigned Wraparound}
...
\begin{frame}{Self-Test: Solutions 1 \& 2}
    ...
    \begin{block}{Solution 2}
        An unsigned $n$-bit container spans $0$ to $2^n - 1$. Adding $1$ to $255$ in an 8-bit container triggers arithmetic wraparound modulo $2^8$, resulting in $0$ and setting the carry flag.
    \end{block}
\end{frame}
