...
\begin{frame}{Unsigned Arithmetic and Fixed-Width Limits}
    ...
    \begin{itemize}
        ...
        \item \key{Truncation \& Modular Behavior}: High-order carry bits exceeding the bit-width are truncated, leaving the remainder value constrained to the modular range $\pmod{2^n}$.
...
\begin{frame}{Signed Arithmetic Overflow}
    \begin{block}{Definition of Signed Overflow}
        Signed overflow occurs when two numbers of the \textbf{same sign} are added and the result produces a number of the \textbf{opposite sign}, falling outside the representable signed range.
    \end{block}
    \vspace{0.3em}
    \begin{itemize}
        \item \key{Structural Symptom}: Adding two positive numbers yields a negative result, or adding two negative numbers yields a positive result.
        \item \key{Detection}: Hardware evaluates carry inputs and outputs into the MSB rather than just looking at the final bit pattern.
...
\begin{frame}{Do Carry and Overflow Represent Error Conditions?}
    \begin{block}{Status Flags vs. Hardware Errors}
        Hardware status flags (\textbf{C} and \textbf{V}) are \textbf{not} automatic error exceptions; they are arithmetic indicators that software can inspect.
    \end{block}
    \vspace{0.3em}
    \begin{itemize}
        \item \key{Carry Flag (C)}: Indicates an unsigned overflow or borrow out of the MSB. 
        \begin{itemize}
            \small
            \item \emph{Error?}: Only an error if the program intended to treat the data strictly as bounded unsigned integers (e.g., in standard multi-word arithmetic or address calculations, carry is expected and used).
        \end{itemize}
        \item \key{Overflow Flag (V)}: Indicates signed wraparound (adding two numbers of the same sign yielded the opposite sign). 
        \begin{itemize}
            \small
            \item \emph{Error?}: Critical for signed integer math, as it flags unintended numeric corruption. However, hardware does not halt; software must explicitly check branches (e.g., \code{b.vs}) to handle it.
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Wrap-Up and Next Steps}
    \begin{block}{Transition to Topic 4}
        \begin{itemize}
            ...
            \item \key{Multiplication and Division}: Hardware mechanics for scaling operations and handling remainders.
            \item \key{Condition Flags}: Practical tracking of carry, overflow, and status indicators in execution flow.
            \item \key{Performance Metrics}: Evaluating clock rate, CPI (Cycles Per Instruction), and total execution time.
        \end{itemize}
    \end{block}
\end{frame}
