...
{\color{gold}\normalsize\bfseries
CSCI 6461 \;|\; Computer Systems Architecture \;|\; TOPIC 6
\par}

\vspace{0.25cm}

{\color{white}\fontsize{24}{28}\selectfont\bfseries
Subroutines, Calling Conventions,\\[0.08cm]\& the Stack
\par}

\vspace{0.35cm}

{\color{white!80}\large
Function linkage, register preservation rules, stack frame management,\\and AAPCS64 architectural compliance
\par}
...
\begin{enumerate}\setlength{\itemsep}{8pt}
  \item \key{Function Calls and Returns}
  \begin{itemize}
    \item Branch-with-link, the link register, returns, leaf functions, and non-leaf functions.
  \end{itemize}

  \item \key{AAPCS64 Register Rules}
  \begin{itemize}
    \item Argument registers, return registers, caller-saved registers, and callee-saved registers.
  \end{itemize}

  \item \key{Stack Frames}
  \begin{itemize}
    \item Stack growth, frame records, frame pointer use, prologues, epilogues, and local storage.
  \end{itemize}

  \item \key{Worked Function Examples}
  \begin{itemize}
    \item Leaf functions, nested calls, recursion, extra arguments, and common linkage mistakes.
  \end{itemize}
\end{enumerate}
...
\begin{tikzpicture}[
  ...
  call/.style={-{Latex},thick,gold},
  ret/.style={-{Latex},thick,navy},
  lab/.style={font=\small\bfseries,navy},
  note/.style={font=\small}
]

% Lifelines
\draw[line] (0,0) -- (0,-4.6);
\draw[line] (5.3,0) -- (5.3,-4.6);

\node[lab] at (0,0.28) {Caller Code};
\node[lab] at (5.3,0.28) {Callee Code};

% Call and return
\draw[call] (0,-1.05) -- node[above,sloped,font=\scriptsize\ttfamily]
  {1. bl callee (x30 <- pc + 4)} (5.3,-1.75);

\draw[ret] (5.3,-3.05) -- node[above,sloped,font=\scriptsize\ttfamily]
  {2. ret (pc <- x30)} (0,-3.75);

% Notes
\node[note,anchor=west] at (5.65,-2.25) {Compute result};
\node[note,anchor=east] at (-0.35,-4.05) {Resume next instruction};
...
\begin{center}
{\Large Control transfers from caller to callee, then returns using \bits{x30}.}
\end{center}
