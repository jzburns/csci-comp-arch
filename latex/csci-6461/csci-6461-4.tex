...
\begin{enumerate}\setlength{\itemsep}{12pt}
  ...
  \begin{itemize}
    ...
    \item Bitwise logic (\code{and}, \code{orr}, \code{eor}, \code{bic}) and common bit-manipulation idioms.
  \end{itemize}

  \item \key{Condition Flags \& Comparisons}
  \begin{itemize}
    \item NZCV flags, comparison instructions (\code{cmp}, \code{tst}), and signed vs.\ unsigned branches.
  \end{itemize}
...
\code{madd x0, x1, x2, x3} & Multiply and add in one instruction. \\
...
\begin{itemize}\setlength{\itemsep}{4pt}
  \item The flags describe the result of the most recent flag-setting operation.
  \item For \code{cmp x0, x1}, read the flags as if the processor computed \code{x0 - x1} and then discarded the result.
\end{itemize}
...
\rowcolors{2}{softgray}{white}
\begin{tabular}{p{0.10\textwidth}p{0.39\textwidth}p{0.42\textwidth}}
\toprule
\textbf{Flag} & \textbf{Meaning} & \textbf{Example} \\
\midrule
\key{N} & Result sign bit is 1. & \code{0 - 1} produces a negative result. \\
\key{Z} & Result is exactly zero. & \code{5 - 5} produces zero. \\
\key{C} & Unsigned carry, or no borrow in subtraction. & \code{5 - 3} sets C; \code{3 - 5} clears C. \\
\key{V} & Signed mathematical result does not fit. & 32-bit \code{0x7fffffff + 1} cannot be represented, so V is set. \\
\bottomrule
\end{tabular}
\end{center}

\vspace{0.12cm}
Signed branch conditions often combine flags. For example, after \code{cmp}, \code{b.lt} tests $\text{N} \neq \text{V}$ because signed overflow can make the raw sign bit misleading.
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{4pt}
  \item Comparison and test instructions are assembler aliases for ordinary flag-setting operations.
...
\vspace{0.16cm}
\footnotesize
\key{Related forms:} \code{cset w0, eq} sets 0 or 1 from a condition; \code{cinc x0, x1, eq} conditionally increments \bits{x1}.
\end{frame}
...
\begin{enumerate}\setlength{\itemsep}{7pt}
  \item \key{Loop Test Placement:} Why do compilers often place the conditional branch after the loop body rather than before it?
