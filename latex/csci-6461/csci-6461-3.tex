...
\begin{enumerate}\setlength{\itemsep}{7pt}
  ...
  \begin{itemize}
    \item Data movement, arithmetic/logical operations, memory access, and basic branch structure.
  \end{itemize}
\end{enumerate}
\vfill
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{6pt}
  ...
  \item Topic 4 gives the full treatment of those flags when they are used for comparisons, branches, and conditional select.
\end{itemize}

\vfill

\begin{block}{Course Boundary}
\small In this topic, \bits{pstate} is introduced as part of the programmer-visible state. The detailed interpretation of \bits{N}, \bits{Z}, \bits{C}, and \bits{V} belongs with AArch64 assembly control flow.
\end{block}
...
\begin{itemize}\setlength{\itemsep}{5pt}
  \item \key{Architectural State:} Precise hardware-software contract defined by general registers (\bits{x0}--\bits{x30}), processor state, stack pointer (\bits{sp}), program counter (\bits{pc}), and byte-addressed memory.
  ...
  \item \key{Control Flow Preview:} Branches update the program counter; conditional flag interpretation is developed in Topic 4.
\end{itemize}
\vfill
\end{frame}
