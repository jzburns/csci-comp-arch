...
\small Parsing variable-length instructions sequentially creates a fundamental thermal and frequency limit in modern superscalar front-ends, consuming massive transistor budgets just to find instruction boundaries.
\end{block}
\vfill
\end{frame}
...
\small Instead of relying on rigid hardware to interpret complex commands, RISC shifts the burden of scheduling, register allocation, and optimization entirely to the compiler.
\end{block}
\vfill
\end{frame}
...
\small By simplifying the logic required to decode and route instructions, processors can redirect massive chip area into larger L1 caches, more execution units, and deeper predictive pipelines.
\end{block}
\vfill
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{4pt}
  ...
  \item Enables simple parallel decoders (e.g., 8-wide decode) by strictly fetching $\text{pc} + 4k$.
\end{itemize}
\end{block}
\end{column}
...
\begin{itemize}\setlength{\itemsep}{4pt}
  ...
  \item Requires sequential pre-decoders just to locate instruction boundaries.
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\small The computational effort of determining where one CISC instruction ends and the next begins requires massive transistor budgets, directly degrading a chip's Performance-per-Watt.
\end{block}
\vfill
\end{frame}
...
\begin{frame}{The Cost of Legacy Backwards Compatibility}
\vfill
\begin{itemize}\setlength{\itemsep}{6pt}
  \item \key{The CISC Decoder Tax:} x86-64 carries decades of binary compatibility baggage (16-bit 8086, 32-bit i386). To execute modern 64-bit commands, it relies on \key{prefix bytes} (e.g., the \hex{48} REX prefix) layered over legacy opcodes.
  \item \key{Sequential Choke Point:} A hardware decoder cannot know an instruction's true length until it reads these prefixes byte-by-byte in real time.
  \item \key{The AArch64 Clean Slate:} ARM deliberately dropped legacy 32-bit instruction compatibility (A32) within its new 64-bit execution state (A64).
  \item \key{The RISC Advantage:} This strict severing of legacy ties guaranteed a clean, zero-prefix 4-byte instruction width, enabling massively parallel instruction fetching without historical penalties.
\end{itemize}
\vfill
\end{frame}
...
\key{ALU Operands}        & 3-operand (\code{add d, s1, s2})       & 2-operand destructive (\code{add dst, src}) \\
\key{Memory Access}       & Load/Store only (\code{ldr}/\code{str})& Register-Memory (\code{add rax, [rbx]}) \\
\key{General Registers}   & 31 orthogonal (\bits{x0}--\bits{x30})  & 16 non-uniform (\bits{rax}, \bits{rbx}\dots) \\
\key{Decode Logic}        & Direct parallel single-cycle           & Sequential scan + $\mu\text{op}$ cache \\
\key{Design Focus}        & Compiler-driven scheduling            & Dynamic hardware translation \\
\key{Primary Benefit}     & Parallel decode \& power efficiency    & Compact binaries \& backwards compatibility \\
\bottomrule
\end{tabular}

\vfill
\begin{block}{Architectural Trade-Off}
\small CISC minimizes binary size and maintains historical software compatibility through complex hardware decoders; RISC simplifies datapath logic to maximize decode throughput and energy efficiency.
\end{block}
\vfill
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{4pt}
  ...
  \item \key{Application to Architecture:} You cannot optimize total CPU performance indefinitely if front-end decode bottlenecks ($1-f$) remain sequentially limited by variable-length processing.
\end{itemize}
\vfill
\end{frame}
...
\small Because modern dense transistors cannot all be powered simultaneously without melting the chip, power efficiency is now the primary limit dictating multi-core processor performance.
\end{block}
\vfill
\end{frame}
...
\vfill
\begin{itemize}\setlength{\itemsep}{8pt}
  \item \key{Complexity Redistribution:} ISA design strictly governs the workload distribution between software compilers and hardware decoders.
  \item \key{CISC Trade-offs:} Maximizes code density via variable-length encodings and register-memory math, at the steep cost of front-end decode bottlenecks and legacy prefix overhead.
  \item \key{RISC Trade-offs:} Enforces pipeline regularity via fixed 32-bit widths and strict load/store rules, dropping backwards compatibility to prioritize low CPI and high parallelism.
  \item \key{Microarchitectural Convergence:} Modern x86 fundamentally validates RISC by internally cracking CISC instructions into $\mu\text{ops}$.
  \item \key{Evaluation Metrics:} True architectural efficiency is evaluated via the Iron Law ($T_{\text{CPU}} = \frac{\mathit{IC} \times \mathit{CPI}}{f_{\text{clk}}}$) and Performance-per-Watt.
\end{itemize}
\vfill
\end{frame}
