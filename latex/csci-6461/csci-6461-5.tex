...
\begin{itemize}\setlength{\itemsep}{4pt}
  ...
  \item Simplifies datapath organization and dependency tracking.
...
\vspace{0.25cm}
\normalsize
Can reach any global data variable within $\pm 4\text{ GB}$ of the current instruction (for the standard small model).
\end{frame}
...
\vspace{0.35cm}
\normalsize
\key{Efficiency Impact:} Cuts the memory instruction count for the shown pair of saves/restores by $50\%$.
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{7pt}
  ...
  \item Violating 16-byte alignment can cause compatibility issues, ABI violations, or runtime faults in certain software environments and exception handlers.
...
\begin{itemize}\setlength{\itemsep}{5pt}
  \item \key{Load/Store Discipline:} Arithmetic operations are strictly decoupled from memory; data transfers are handled by load/store-family instructions.
