...
\toprule
\rowcolor{white}
\textbf{Access Pattern} & \textbf{Addressing Form} & \textbf{C Pointer Pattern} & \textbf{Typical Use} \\
\midrule
Fixed offset from a base pointer & \code{[xn, \#imm]} & \code{*(ptr + k)} & Struct fields, known offsets \\
Step through consecutive elements & \code{[xn], \#imm} & \code{*ptr++} & Array traversal \\
Base plus scaled index & \code{[xn, xm, lsl \#k]} & \code{arr[i]} & Indexed array access \\
\bottomrule
\end{tabular}
\end{center}
...
\vspace{0.18cm}
\normalsize
C pointer notation describes the same idea: start from a base address, optionally add an offset or index, then dereference.
\end{frame}
...
\vspace{0.3cm}
\begin{block}{Example: Reading Values at Fixed Offsets}
\small
\begin{tabular}{lll}
\textbf{Assembly} & \textbf{C-like pattern} & \textbf{Meaning} \\
\code{ldr x1, [x0, \#0]}  & \code{*ptr}        & Read value at the base address \\
\code{ldr x2, [x0, \#8]}  & \code{*(ptr + 1)}  & Read the next 64-bit element \\
\code{ldr w3, [x0, \#16]} & \code{*(base + 16)} & Read a 32-bit value at byte offset 16 \\
\end{tabular}
\end{block}
\end{frame}
...
The offset tells the instruction where the desired object is relative to the base pointer. In C terms, this is the same idea as \code{*(ptr + k)}, where \code{k} selects an element relative to the base pointer.
\end{block}
\end{frame}
...
\code{ldr x0, [x1, \#-8]!} \quad $\approx$ \quad \code{*\char45{}\char45{}ptr} \\
\text{\scriptsize Moves the pointer first, then loads from the updated address.}
\end{block}
\end{frame}
...
\code{ldr x0, [x1], \#8} \quad $\approx$ \quad \code{*ptr++} \\
\text{\scriptsize Loads from the current pointer, then advances the pointer to the next 8-byte element.}
\end{block}
\end{frame}
...
\toprule
\rowcolor{white}
\textbf{Mode} & \textbf{Syntax} & \textbf{C Pattern} & \textbf{Address Accessed} & \textbf{Base After} \\
\midrule
\textbf{Standard Offset} & \code{[xn, \#8]}  & \code{*(ptr + 1)} & $x_n + 8$ & Unchanged \\
\textbf{Pre-Indexed}     & \code{[xn, \#8]!} & \code{*++ptr}     & $x_n + 8$ & Updated \\
\textbf{Post-Indexed}    & \code{[xn], \#8}  & \code{*ptr++}     & $x_n$     & Updated \\
\bottomrule
\end{tabular}
\end{center}
...
\small
\begin{tabular}{lll}
\code{ldr x0, [x1, \#8]}  & Reads from \hex{1008}; & $x_1$ remains \hex{1000} \\
\code{ldr x0, [x1, \#8]!} & Reads from \hex{1008}; & $x_1$ becomes \hex{1008} \\
\code{ldr x0, [x1], \#8}  & Reads from \hex{1000}; & $x_1$ becomes \hex{1008} \\
\end{tabular}
\end{block}
\end{frame}
...
\begin{itemize}\setlength{\itemsep}{6pt}
  ...
  \item This directly corresponds to the C pattern \code{arr[i]}.
\end{itemize}
...
\toprule
\rowcolor{white}
\textbf{Element Type} & \textbf{C Pattern} & \textbf{Assembly Instruction} & \textbf{Effective Address} \\
\midrule
Byte      & \code{arr[i]} & \code{ldrb w0, [x1, x2]}          & $x_1 + x_2$ \\
Halfword  & \code{arr[i]} & \code{ldrh w0, [x1, x2, lsl \#1]} & $x_1 + (x_2 \times 2)$ \\
Word      & \code{arr[i]} & \code{ldr  w0, [x1, x2, lsl \#2]} & $x_1 + (x_2 \times 4)$ \\
Doubleword& \code{arr[i]} & \code{ldr  x0, [x1, x2, lsl \#3]} & $x_1 + (x_2 \times 8)$ \\
\bottomrule
\end{tabular}
\end{center}
...
\vspace{-0.2cm}
\small \key{Goal:} Sum $N$ 64-bit Integers \hfill \key{C pattern:} \code{sum += *ptr++}
\vspace{-0.1cm}
...
\vspace{-0.15cm}
\normalsize
Post-indexing combines the load and pointer advance in one instruction.
\end{frame}
...
\small \key{Goal:} Check Element at Index $i$ in Array \hfill \key{C pattern:} \code{arr[i]}
...
\rowcolors{2}{softgray}{white}
\begin{tabular}{>{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{2.35cm} >{\raggedright\arraybackslash}p{3.0cm} >{\raggedright\arraybackslash}p{4.2cm}}
\toprule
\rowcolor{white}
\textbf{Programming Scenario} & \textbf{C Pattern} & \textbf{Addressing Mode} & \textbf{Example Instruction} \\
\midrule
Accessing a known offset & \code{*(ptr + k)} & Base + immediate offset & \code{ldr x1, [x0, \#8]} \\
Loading the next element & \code{*ptr++} & Post-indexed update & \code{ldr x2, [x0], \#8} \\
Stepping backward through data & \code{*\char45{}\char45{}ptr} & Pre-indexed update & \code{ldr x2, [x0, \#-8]!} \\
Accessing an indexed element & \code{arr[i]} & Scaled register offset & \code{ldr x2, [x0, x1, lsl \#3]} \\
Moving two adjacent values & \code{ptr[0], ptr[1]} & Paired transfer & \code{ldp x2, x3, [x0]} \\
\bottomrule
\end{tabular}
\end{center}
\end{frame}
